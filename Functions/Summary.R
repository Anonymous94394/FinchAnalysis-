summarise_data <- function(data, group_var, Measurement, caption) {
  ...
  knitr::kable(
    ...
    col.names = c(group_var, "Mean (mm)", "SD (mm)"), #Collumn names 
    digits = 3  # Round to 3 decimal places
  ) %>%
    kable_styling(bootstrap_options = c("striped", "hover")) #Format 
}